...
| D-096 | Changing the pinned trusted project identity used to authenticate updates requires explicit approval and a recorded rotation. A rotation is never accepted from the update channel itself and is never inferred from a newly presented signature. | The pinned identity is the whole basis of `D-018`'s publisher authentication: an attacker who can silently install a new pinned identity becomes the publisher, and every later signature verifies correctly. Approval is required because rotation is a change to key handling, which [Data and privacy](DATA_AND_PRIVACY.md) §11 and [Compatibility](COMPATIBILITY.md) §5 already place behind approval. Ratified 2026-08-21 after Wave D flagged `SC-PUB-003` as derived rather than quoted. |
