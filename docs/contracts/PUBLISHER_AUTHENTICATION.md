...
| `SC-PUB-003` | The pinned trusted project identity changes only through an approved, recorded rotation. A rotation must not be accepted from the update channel itself, must not be inferred from a newly presented signature, and — because it is a change to key handling — requires explicit approval as a protected action under `SC-CONF-001`. | Refusing an unapproved identity change. | [`D-096` in the decision register](../DECISION_REGISTER.md), [Data and privacy §11](../DATA_AND_PRIVACY.md#11-updates-repair-and-failure-behavior), [Compatibility §5](../COMPATIBILITY.md#5-relationship-to-automatic-updates), [`D-018` in the decision register](../DECISION_REGISTER.md) |
...
| `SC-PUB-006` | A failed verification must install nothing. The artifact must be retained as evidence rather than silently discarded, and no partial installation may remain. The failure must be recorded so that it is available for review; **how the user is told is deliberately not fixed here** — notification design belongs to Stage 6, and this contract does not pre-empt it. | Installing nothing on failure. | [Architecture §Security and trust boundaries](../ARCHITECTURE.md#security-and-trust-boundaries), [Architecture §Expected failure modes](../ARCHITECTURE.md#expected-failure-modes) |
...
- **Signature or attestation missing, malformed, or unverifiable.** Failed verification; nothing installs and the artifact is retained (`SC-PUB-002`, `SC-PUB-006`).
...
**Deliberately not owed here.** How a verification failure reaches the user — a Windows notification, a health view, a queued review item — is Stage 6's notification design, not this contract's. `SC-PUB-006` originally carried a warning requirement derived by analogy from [Architecture §Operations](../ARCHITECTURE.md#operations), which reserves notifications for urgent conditions but does not name signature verification among them. That clause was narrowed away on 2026-08-21 rather than frozen on an analogy: freezing it here would have settled a Stage 6 question inside a Stage 0 security contract, and the security-critical halves — nothing installs, evidence is retained — are quoted rather than inferred and are unaffected.
